...
public class MsgController {
     ...
     * @throws AesException
     */
    @PostMapping(value = "/receiveMsg", produces = MediaType.APPLICATION_XML_VALUE)
    public String res(String msg_signature, String timestamp, String nonce, HttpServletRequest request) throws IOException, AesException {
        ...
        String Content = null;
        try {
            Map<String, String> mMsg = wxcpt.myDecryptMsg(msg_signature, timestamp, nonce, postData);
//            String sMsg = wxcpt.DecryptMsg(msg_signature, timestamp, nonce, postData);
            String sMsg = mMsg.get("result");
            String encrypt = mMsg.get("encrypt");
            System.out.println("after decrypt msg: " + mMsg.get("result"));
            ...
            int myNonce = random.nextInt(900000000) + 100000000;
            Content = "<xml>\n" +
                    "   <Encrypt><![CDATA[" + encrypt + "]]></Encrypt>\n" +
                    "   <MsgSignature><![CDATA[" + msg_signature + "]]></MsgSignature>\n" +
                    "   <TimeStamp>" + newTimeSTAMP + "</TimeStamp>\n" +
                    "   <Nonce><![CDATA[" + myNonce + "]]></Nonce>\n" +
                    "</xml>";
    ...
    @PostMapping(value = "/demo", produces = MediaType.APPLICATION_XML_VALUE)
    public String demo(String msg_signature, String timestamp, String nonce, HttpServletRequest request) throws IOException, AesException {
        long newTimeSTAMP = System.currentTimeMillis();
        Random random = new Random();
        int myNonce = random.nextInt(900000000) + 100000000;
        String encrypt = "alsfaslkjkasdfkjlsfadjklasfdjkl";
        String Content = "<xml>\n" +
                "   <Encrypt><![CDATA[" + encrypt + "]]></Encrypt>\n" +
                "   <MsgSignature><![CDATA[" + msg_signature + "]]></MsgSignature>\n" +
                "   <TimeStamp>" + newTimeSTAMP + "</TimeStamp>\n" +
